...
-- @param x table
-- @return table
function dict.copy(x) return utils.copy(x) end
...
function dict.keys(t, sort, cmp)
  ...
  if sort then return array.sort(out, cmp) end
...
-- @param t dict
-- @return boolean
function dict.isblank(t) return #dict.keys(t) == 0 end
...
-- @param t dict
-- @return length
function dict.len(t) return #dict.keys(t) end
...
function dict.update(tbl, keys, value) return array.update(tbl, keys, value) end
...
function dict.index(t, item, test, depth)
  depth = depth or -1
  test = test or function(x, y) return x == y end
  local cache = {}

  local function _index(x, d, layer)
    if cache[x] then return end
...
-- @return any
function dict.get(tbl, ks, create_path) return array.get(tbl, ks, create_path) end

--- Get element at path specified by key(s)
-- @param tbl dict|table
-- @param ... key|dict[keys] to get the element from
-- @return any
function dict.contains(tbl, ...) return (dict.get(tbl, { ... })) end

--- Create a table at the path specified by key(s). Modifies the dict
-- @param t dict
-- @param ... key(s) where the new table shall be made
-- @return dict
function dict.makepath(t, ...) return dict.get(t, { ... }, true) end
...
function dict.lmerge(...)
  ...
  local function _merge(t1, t2)
    ...
    array.each(later, function(next) _merge(unpack(next)) end)
  end

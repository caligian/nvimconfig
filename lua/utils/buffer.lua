...
function Buffer.setwinvars(self, vars)
  ...
  teach(vars, partial(self.setwinvar, self))
...
function Buffer.split(self, split, opts)
  ...
  validate {
    split_direction = { "s", split },
    split_options = { "t", opts },
  }
...
function Buffer.menu(desc, items, formatter, callback)
  ...
  b:hook("WinLeave", partial(b.delete, b))
  b:bind(
    { noremap = true, event = "BufEnter" },
    { "q", partial(b.delete, b) },
...
function Buffer.input(text, cb, opts)
  ...
  buf:noremap("n", "gq", partial(buf.hide, buf), "Close buffer")

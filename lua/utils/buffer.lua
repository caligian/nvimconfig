...
local function update(self)
  ...
  if self.scratch then V.update(Buffer.scratch, { self.bufnr }, self) end
end
...
function Buffer._init(self, name, scratch)
  ...
  if Buffer.bufnr[bufnr] then return Buffer.bufnr[bufnr] end
...
function Buffer.text(self, start, till, repl)
  ...
  if types.is_type(repl) == 'string' then repl = vim.split(repl, '[\n\r]') end
...
function Buffer.switch(self) vim.cmd('b ' .. self.bufnr) end

function Buffer.load(self) vim.fn.bufload(self.bufnr) end

function Buffer.loaded(self) return vim.fn.bufloaded(self.bufnr) ~= 0 end
...
function Buffer.input(name, text, cb, split, trigger_keys)
  ...
  if V.is_type(text, 'string') then text = vim.split(text, '\n') end
...
function Buffer.getmap(self, mode, lhs) return V.buffer_has_keymap(self.bufnr, mode, lhs) end

function Buffer.range(self) return V.get_visual_range(self.bufnr) end

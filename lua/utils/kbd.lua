...
local function parse_opts(opts)
  ...
  V.teach(opts, function(k, v)
    ...
    if V.match(k, "pattern", "once", "nested", "group") then
      parsed.au[k] = v
    elseif V.match(k, "event", "name", "mode", "prefix", "leader", "localleader", "cond") then
      parsed.misc[k] = v
    else
      parsed.kbd[k] = v
    end
...
new 'Keybinding' {
  ...
  update = function(self)
    V.update(Keybinding.ids, self.id, self)
  ...
  -- @usage K(mode, lhs, cb, {
    ...
    -- @see autocmd
    -- @return object
    _init = function(self, mode, lhs, cb, rest)
      ass.st(mode, "mode")
      ass.s(lhs, "lhs")
      ass.fs(cb, "callback")
    ...
    noremap = function(mode, lhs, cb, opts)
      opts = opts or {}

...
function Keybinding.update(self)
  ...
  if self.bufnr then V.update(Keybinding.buffer, { self.bufnr, self.id }, self) end

  return self
end

local function getkbdopts(opts)
  if V.isstring(opts) then return { desc = opts } end
...
function Keybinding._init(self, mode, lhs, cb, rest)
  ...
  if V.isstring(mode) then mode = vim.split(mode, '') end
...
function Keybinding.bind(opts, ...)
  ...
  for _, kbd in ipairs({ ... }) do
    ...
    if kbd[3] then
      if V.isstring(kbd[3]) then kbd[3] = { desc = kbd[3] } end
    end
...
function Keybinding.map(mode, lhs, cb, opts) return Keybinding(mode, lhs, cb, opts) end

...
local function is_ancestor_of(parent, child)
  local mt_p, mt_c = get_mt(parent), get_mt(child)

  if not mt_p and not mt_c then
    return false
  end
  ...
  for key, value in pairs(t) do
    if is_callable(value) then
      obj[key] = function (inst, ...)
        return value(obj[use_attrib] or obj, ...)
      end
    else
      obj[key] = value
    end

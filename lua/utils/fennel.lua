	...
	or function(...)
			...
			do
				...
				for name in pairs(env.___replLocals___) do
					...
					if nil ~= val_18_auto then
						...
						(tbl_16_auto)[i_17_auto] = val_18_auto
					else
					end
				end
				saves = tbl_16_auto
			...
			local function add_partials(input, tbl, prefix)
				...
				for k, is_mangled in utils.allpairs(_644_()) do
					...
					do
						...
						if
							(type(k) == "string")
							and (input == k:sub(0, #input))
							and not seen[k]
							and ((":" ~= prefix:sub(-1)) or ("function" == type(tbl[lookup_k])))
						then
							seen[k] = true
		...
		commands.find = function(env, read, on_values, on_error, scope)
			local function _695_(_241)
				...
				if
					(_G.type(_696_) == "table")
					and ((_696_).what == "Lua")
					and (nil ~= (_696_).source)
					and (nil ~= (_696_).linedefined)
					and (nil ~= (_696_).short_src)
				then
		...
		commands.doc = function(env, read, on_values, on_error, scope)
			local function _706_(_241)
				...
				local function _707_()
					return (
						utils["get-in"](scope.specials, path)
						or utils["get-in"](scope.macros, path)
		...
		commands.compile = function(env, read, on_values, on_error, scope)
			local function _709_(_241)
				...
				local ok_3f, result =
					pcall(compiler.compile, _241, { env = env, scope = scope, allowedGlobals = allowedGlobals })
				if ok_3f then
	...
	or function(...)
		...
		local function doc_2a(tgt, name)
			if not tgt then
				...
				local docstring = (((compiler.metadata):get(tgt, "fnl/docstring") or "#<undocumented>"))
					:gsub("\n$", "")
					:gsub("\n", "\n  ")
				local mt = getmetatable(tgt)
				if (type(tgt) == "function") or ((type(mt) == "table") and (type(mt.__call) == "function")) then
					local arglist =
						table.concat(((compiler.metadata):get(tgt, "fnl/arglist") or { "#<unknown-arguments>" }), " ")
					local _438_
					if 0 < #arglist then
						_438_ = " "
					else
		...
		local function doc_special(name, arglist, docstring, body_form_3f)
			compiler.metadata[SPECIALS[name]] =
				{ ["fnl/arglist"] = arglist, ["fnl/docstring"] = docstring, ["fnl/body-form?"] = body_form_3f }
			return nil
		end
		...
		local function insert_meta(meta, k, v)
			...
			compiler.assert(
				literal_3f(v),
				("expected literal value in metadata table, got: %s %s"):format(view(k, view_opts), view(v, view_opts))
			)
			local _452_ = meta
		...
		SPECIALS.each = function(ast, scope, parent)
			...
			compiler.emit(
				parent,
				("for %s in %s do"):format(table.concat(bind_vars, ", "), table.concat(val_names, ", ")),
				ast
			)
		...
		local function for_2a(ast, scope, parent)
			...
			compiler.assert(
				...
				("unable to bind %s %s"):format(type(binding_sym), tostring(binding_sym)),
				ast[2]
			)
			compiler.assert((3 <= #ast), "expected body expression", ast[1])
		...
		local function native_method_call(ast, _scope, _parent, target, args)
			...
			return utils.expr(
				string.format(call_string, tostring(target), method_string, table.concat(args, ", ")),
				"statement"
			)
		end
		local function nonnative_method_call(ast, scope, parent, target, args)
			...
			return utils.expr(
				string.format("%s[%s](%s)", tostring(target), method_string, table.concat(args0, ", ")),
				"statement"
			)
		end
		...
		SPECIALS.hashfn = function(ast, scope, parent)
			...
			local function walker(idx, node, parent_node)
				if utils["sym?"](node) and (tostring(node) == "$...") then
					...
					return (
						("table" == type(node))
						and (utils.sym("hashfn") ~= node[1])
						and (utils["list?"](node) or utils["table?"](node))
					)
				end
			end
		...
		local function maybe_short_circuit_protect(ast, i, name, _518_)
			...
			if
				(("or" == name) or ("and" == name))
				and (1 < i)
				and (mac[call] or ("set" == call) or ("tset" == call) or ("global" == call))
			then
		...
		SPECIALS["require-macros"] = function(ast, scope, parent, _3freal_ast)
			...
			if not macro_loaded[modname] then
				...
				(macro_loaded)[modname] = compiler.assert(
					utils["table?"](loader(modname, filename)),
		...
		SPECIALS.include = function(ast, scope, parent, opts)
			...
			if (modexpr.type ~= "literal") or ((modexpr[1]):byte() ~= 34) then
				...
				res = (
					(utils["member?"](mod, (utils.root.options.skipInclude or {})) and opts.fallback(modexpr, true))
	...
	or function(...)
		...
		local function make_scope(_3fparent)
			...
			return {
				...
				manglings = setmetatable({}, { __index = (parent and parent.manglings) }),
				specials = setmetatable({}, { __index = (parent and parent.specials) }),
				symmeta = setmetatable({}, { __index = (parent and parent.symmeta) }),
				unmanglings = setmetatable({}, { __index = (parent and parent.unmanglings) }),
				gensyms = setmetatable({}, { __index = (parent and parent.gensyms) }),
				autogensyms = setmetatable({}, { __index = (parent and parent.autogensyms) }),
		...
		local function check_binding_valid(symbol, scope, ast, _3fopts)
			...
			assert_compile(not name:find("^%."), "invalid character: .", symbol)
			assert_compile(
				not (scope.specials[name] or (not macro_3f and scope.macros[name])),
		...
		local function symbol_to_expression(symbol, scope, _3freference_3f)
			...
			assert_compile(
				(not scope.specials[parts[1]] or ("require" == parts[1])),
				"tried to reference a special form without calling it",
				symbol
			)
			assert_compile(
				(not _3freference_3f or local_3f or ("_ENV" == parts[1]) or global_allowed_3f(parts[1])),
				("unknown identifier: " .. tostring(parts[1])),
				symbol
			)
		...
		local function find_macro(ast, scope)
			...
			if not macro_2a and multi_sym_parts then
				...
				assert_compile(
					(not scope.macros[multi_sym_parts[1]] or (type(nested_macro) == "function")),
		...
		local function compile_function_call(ast, scope, parent, opts, compile1, len)
			...
			assert_compile(
				(utils["sym?"](ast[1]) or utils["list?"](ast[1]) or ("string" == type(ast[1]))),
		...
		local function compile_call(ast, scope, parent, opts, compile1)
			...
			elseif multi_sym_parts and multi_sym_parts["multi-sym-method-call"] then
				...
				local new_ast = utils.list(
					utils.sym(":", ast),
					utils.sym(table_with_method, ast),
					method_to_call,
					select(2, unpack(ast))
				)
		...
		local function destructure(to, from, ast, scope, parent, opts)
			...
			local function getname(symbol, up1)
				...
				if declaration then
					...
					assert_compile(
						(meta or not opts0.noundef or global_allowed_3f(parts[1])),
		...
		local function compile_stream(strm, options)
			...
			for i = 1, #vals do
				local exprs =
					compile1(vals[i], scope, chunk, { nval = (((i < #vals) and 0) or nil), tail = (i == #vals) })
		...
		local function traceback(_3fmsg, _3fstart)
			...
			if
				(msg:find("^%g+:%d+:%d+ Compile error:.*") or msg:find("^%g+:%d+:%d+ Parse error:.*"))
		...
		return {
			...
			["check-binding-valid"] = check_binding_valid,
			emit = emit,
...
package.preload["fennel.friend"] = package.preload["fennel.friend"]
	or function(...)
		...
		local suggestions = {
			...
			["tried to reference a macro without calling it"] = {
			...
			["unexpected iterator clause"] = { "removing an argument", "checking for typos" },
		}
		local unpack = (table.unpack or _G.unpack)
		...
		local function sub(str, start, _end)
			...
			elseif utf8_ok_3f then
				return string.sub(
					...
					((utf8.offset(str, (_end + 1)) or (utf8.len(str) + 1)) - 1)
				)
			else
		...
		local function assert_compile(condition, msg, ast, source, opts)
			if not condition then
				...
				error(
					friendly_msg(
						("%s:%s:%s Compile error: %s"):format((filename or "unknown"), (line or "?"), (col or "?"), msg),
		...
		local function parse_error(msg, filename, line, col, source, opts)
			return error(
				friendly_msg(
					("%s:%s:%s Parse error: %s"):format(filename, line, col, msg),
...
package.preload["fennel.parser"] = package.preload["fennel.parser"]
	or function(...)
		...
		local function char_starter_3f(b)
			return (
				(function(_208_, _209_, _210_)
					return (_208_ < _209_) and (_209_ < _210_)
				end)(1, b, 127)
				or (function(_211_, _212_, _213_)
					return (_211_ < _212_) and (_212_ < _213_)
				end)(192, b, 247)
			)
		end
		local function parser_fn(getbyte, filename, _214_)
			...
			local function whitespace_3f(b)
				...
				return (
					(b == 32)
					or (function(_221_, _222_, _223_)
						return (_221_ <= _222_) and (_222_ <= _223_)
					end)(9, b, 13)
					or _225_()
				)
			end
			...
			local function parse_stream()
				...
				local function badend()
					...
					return parse_error(
						string.format("expected closing delimiter%s %s", _232_, string.char(unpack(accum)))
					)
				end
				...
				local function open_table(b)
					...
					return table.insert(
						stack,
						{ bytestart = byteindex, closer = delims[b], filename = filename, line = line, col = (col - 1) }
					)
				end
...
package.preload["fennel.view"] = package.preload["fennel.view"]
	or function(...)
		...
		local function get_default(key)
			...
			if _7_ == nil then
				return error(
					("option '%s' doesn't have a default value, use the :after key to set it"):format(tostring(key))
				)
			elseif nil ~= _7_ then
				local v = _7_
				return v
		...
		local function concat_table_lines(elements, options, multiline_3f, indent, table_type, prefix, last_comment_3f)
			...
			if
				not getopt(options, "one-line?")
				and (multiline_3f or (options["line-length"] < (indent + length_2a(oneline))) or last_comment_3f)
			then
				local function _43_()
			...
			if length_2a(lines) == 0 then
				...
				if
					...
					and (
						force_multi_line_3f
						or oneline:find("\n")
						or (options["line-length"] < (indent + length_2a(oneline)))
					)
				then
					return table.concat(lines, ("\n" .. string.rep(" ", indent)))
				else
		...
		local utf8_inits = {
			{ ["min-byte"] = 0, ["max-byte"] = 127, ["min-code"] = 0, ["max-code"] = 127, len = 1 },
			{ ["min-byte"] = 192, ["max-byte"] = 223, ["min-code"] = 128, ["max-code"] = 2047, len = 2 },
			{ ["min-byte"] = 224, ["max-byte"] = 239, ["min-code"] = 2048, ["max-code"] = 65535, len = 3 },
			{ ["min-byte"] = 240, ["max-byte"] = 247, ["min-code"] = 65536, ["max-code"] = 1114111, len = 4 },
		}
			...
			local function validate_utf8(str0, index)
				...
				do
					...
					for _, init0 in ipairs(inits) do
						...
						ret = (
							byte
							and (function(_80_, _81_, _82_)
								return (_80_ <= _81_) and (_81_ <= _82_)
							end)(init0["min-byte"], byte, init0["max-byte"])
							and init0
						)
					end
					init = ret
				...
				local function _83_()
					...
					for i = (index + 1), (index + init.len + -1) do
						...
						code0 = (
							byte0
							and code0
							and (function(_85_, _86_, _87_)
								return (_85_ <= _86_) and (_86_ <= _87_)
							end)(128, byte0, 191)
				...
				if
					code
					and (function(_88_, _89_, _90_)
						return (_88_ <= _89_) and (_89_ <= _90_)
					end)(init["min-code"], code, init["max-code"])
					and not (function(_91_, _92_, _93_)
						return (_91_ <= _92_) and (_92_ <= _93_)
					end)(55296, code, 57343)
				then
					return init.len
				else
					return nil
		...
		local function pp_string(str, options, indent)
			...
			local esc_newline_3f = (
				(len < 2) or (getopt(options, "escape-newlines?") and (len < (options["line-length"] - indent)))
			)
			local escs
			local _97_
...
package.preload["fennel.utils"] = package.preload["fennel.utils"]
	or function(...)
		...
		local function luajit_vm_3f()
			return (
				(nil ~= _G.jit)
				and (type(_G.jit) == "table")
				and (nil ~= _G.jit.on)
				and (nil ~= _G.jit.off)
				and (type(_G.jit.version_num) == "number")
			)
		end
		...
		local function fengari_vm_3f()
			return (
				(nil ~= _G.fengari)
				and (type(_G.fengari) == "table")
				and (nil ~= _G.fengari.VERSION)
				and (type(_G.fengari.VERSION_NUM) == "number")
			)
		end
		...
		local function idempotent_expr_3f(x)
			return (
				...
				or (type(x) == "integer")
				or (type(x) == "number")
				or (sym_3f(x) and not multi_sym_3f(x))
			)
		end
		...
		local propagated_options =
			{ "allowedGlobals", "indent", "correlate", "useMetadata", "env", "compiler-env", "compilerEnv" }
		...
		return {
			...
			["macro-path"] = table.concat(
				{ "./?.fnl", "./?/init-macros.fnl", "./?/init.fnl", getenv("FENNEL_MACRO_PATH") },
				";"
			),
		}
	end
...
local function syntax()
	...
	local binding_3f =
		{ "collect", "icollect", "fcollect", "each", "for", "let", "with-open", "accumulate", "faccumulate" }

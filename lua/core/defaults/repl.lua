...
local function parse_args(args)
    return array.map(vim.split(args.args, " +"), function(x)
        if x:match "^[0-9]+%.?[0-9]*$" then x = tonumber(x) end
        return x
    end)
end
...
repl.commands = {
    ...
    stop = {
        function(opts)
            with_args(opts, function(args) repl.stop(unpack(args)) end)
        end,
        { nargs = "?" },
    },
    centerFloat = {
...
repl.mappings = {
    ...
    buffer_send_line = {
        "<leader>re",
        function() vim.cmd(":ReplSendLine " .. buffer.bufnr()) end,
        { desc = "send range (visual)" },
    },
    buffer_send_buffer = {
        "<leader>rb",
        function() vim.cmd(":ReplSendBuffer " .. buffer.bufnr()) end,
        { desc = "send buffer" },
    },
    buffer_start = {
        "<leader>rr",
        function() vim.cmd(":ReplStart " .. buffer.bufnr()) end,
        { desc = "start" },
    },
    buffer_hide = {
        "<leader>rk",
        function() vim.cmd(":ReplHide " .. buffer.bufnr()) end,
        { desc = "hide" },
    },
    buffer_stop = {
        "<leader>rq",
        function() vim.cmd(":ReplStop " .. buffer.bufnr()) end,
        { desc = "stop" },
    },
    buffer_vsplit = {
        "<leader>rv",
        function() vim.cmd(":ReplVsplit " .. buffer.bufnr()) end,
        { desc = "vsplit" },
    },
    buffer_split = {
        "<leader>rs",
        function() vim.cmd(":ReplSplit " .. buffer.bufnr()) end,
        { desc = "split" },
    },
    buffer_dock = {
        "<leader>rd",
    ...
    buffer_center_float = {
        "<leader>rF",
        function() vim.cmd(":ReplCenterFloat " .. buffer.bufnr()) end,
    ...
    filetype_send_line = {
        ...
        function()
            vim.cmd(
                ":ReplSendLine " .. buffer.option(buffer.bufnr(), "filetype")
            )
        end,
        { desc = "send range (visual)" },
    },
    filetype_send_buffer = {
        ...
        function()
            vim.cmd(
                ":ReplSendBuffer " .. buffer.option(buffer.bufnr(), "filetype")
            )
        end,
        { desc = "send buffer" },
    },
    ...
    filetype_center_float = {
        ...
        function()
            vim.cmd(
                ":ReplCenterFloat " .. buffer.option(buffer.bufnr(), "filetype")
            )
        end,
        { desc = "float in middle" },
    },
    ...
    shell_send_line = {
        "<leader>xe",
        function() vim.cmd(":ReplSendLine " .. 'sh') end,
        { desc = "send range (visual)" },
    },
    shell_send_buffer = {
        "<leader>xb",
        function() vim.cmd(":ReplSendBuffer " .. 'sh') end,
        { desc = "send buffer" },
    },
    shell_start = {
        "<leader>xx",
        function() vim.cmd(":ReplStart " .. 'sh') end,
        { desc = "start" },
    },
    shell_hide = {
        "<leader>xk",
        function() vim.cmd(":ReplHide " .. 'sh') end,
        { desc = "hide" },
    },
    shell_stop = {
        "<leader>xq",
        function() vim.cmd(":ReplStop " .. 'sh') end,
        { desc = "stop" },
    },
    shell_vsplit = {
        "<leader>xv",
        function() vim.cmd(":ReplVsplit " .. 'sh') end,
        { desc = "vsplit" },
    },
    shell_split = {
        "<leader>xs",
        function() vim.cmd(":ReplSplit " .. 'sh') end,
        { desc = "split" },
    },
    shell_dock = {
        "<leader>xd",
    ...
    shell_center_float = {
        "<leader>xF",
        function() vim.cmd(":ReplCenterFloat " .. 'sh') end,
        { desc = "float in middle" },
    },
    shell_float = {
        "<leader>xf",

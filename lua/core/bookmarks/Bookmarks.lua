...
function Bookmarks.exists(p)
  return Bookmarks.BOOKMARKS[p] or false
end
...
function Bookmarks.remove(p, line)
  ...
  local ok = get_call(p, function(obj, line)
    obj:remove(line)
  end)
  if ok then
    return ok(line)
...
function Bookmarks.update_context(p)
  if not p then
    dict.each(Bookmarks.BOOKMARKS, function(_, obj)
      obj:update_context()
    end)
    return true
  end

  local ok = get_call(p, function(obj)
    obj:update_context()
  end)
  if ok then
    return ok()
...
function Bookmarks.list(p, telescope)
  ...
  if dict.isblank(Bookmarks.BOOKMARKS) then
    return
  end
...
function Bookmarks.create_picker(p, remover)
  ...
  if exists then
    return exists:create_picker(remover)
  end
end

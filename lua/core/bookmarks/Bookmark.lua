...
function Bookmark:init(p)
  p = resolve_path(p)
  if not p then error("Invalid path supplied " .. p) end
  table.merge(self, p)
end
...
function Bookmark:is_invalid(line)
  if not line then
    if not path.exists(self.path) then return true end
    return false
  end
...
function Bookmark:create_picker(remover)
  ...
  return _.new(ls, {
    ...
    prompt_title = remover and ("Remove marks: " .. self.path)
      or ("Manage marks: " .. self.path),
  })
end

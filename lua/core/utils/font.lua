...
function font.get(name, callback)
	...
	if exists and callback then callback(exists) end
...
function mod:tostring()
  ...
  dict.each(self:get_attribs(), function(key, value) out[i] = ":" .. key .. value end)

  return array.join(out, "")
end

function font.new(family, attribs)
  if not family then font.exception.invalid_font:throw() end
...
function mod:set() vim.o.guifont = self:tostring() end
function mod:is_current() return vim.o.guifont == self:tostring() end
...
function mod:dec(by) return self:inc(-1 * by) end

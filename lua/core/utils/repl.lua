...
function repl.if_running(bufnr_or_filetype, on_success, on_failure)
	...
	if not found or not found:is_running(80) then
		if on_failure then
			return on_failure(bufnr_or_filetype)
		end
...
function repl.stop(bufnr_or_filetype)
	return repl.if_running(bufnr_or_filetype, function(x)
		x:stop()
	end)
end

function repl.stop_all()
	dict.each(repl.repls, function(bufnr, x)
		x:stop()
	end)
	dict.each(repl.single_repls, function(bufnr, x)
		x:stop()
	end)
end
...
function repl.float(bufnr, opts)
	return repl.if_running(bufnr, function(x)
		x:float(opts)
	end)
end

function repl.split(bufnr, direction)
	return repl.if_runnnig(bufnr, function(x)
		x:split(direction)
	end)
end

...
function Plugin.loadall()
  ...
  lazy.setup(
    array.map(dict.values(state), function(x)
      return x.spec
    end),
    { lazy = true }
  )
...
function Plugin:init(name, conf)
  ...
  conf.spec.config = conf.spec.config or function()
    self:load()
  end
  self.name = name
...
plugin = setmetatable({}, {
  __call = function(self, name)
    return function(conf)
      return Plugin.create(name, { spec = conf })
    end
  end,

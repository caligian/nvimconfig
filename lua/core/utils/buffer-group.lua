...
buffer_group.map_commands = function(commands)
	dict.each(commands or buffer_group.commands, function(name, spec)
		buffer_group.command_group:add(name, unpack(spec))
	end)
end
	...
	local self = {
		...
		remove_buffer = function(self, ...)
			...
			array.each({ ... }, function(bufnr)
				...
				local exists, exists_t = dict.get(buffer_group.buffers, { bufnr, self.name })
		...
		enable = function(self)
			...
			if self.autocmds[au_name] and self.autocmds[au_name]:is_enabled() then
		...
		create_picker = function(self, tp)
			local function create_include_buffer_picker()
				...
				function mod.include_buffer(prompt_bufnr)
					self:include_buffer(unpack(array.map(_:get_selected(prompt_bufnr), function(buf)
						return buf.bufnr
					end)))
				end
			...
			local function create_picker(remove)
				...
				function mod.exclude_buffer(prompt_bufnr)
					self:exclude_buffer(unpack(array.map(_:get_selected(prompt_bufnr), function(buf)
						return buf.bufnr
					end)))
				end
...
function buffer_group.create_picker(bufnr)
	...
	function mod.change_pattern(prompt_bufnr)
		...
		group.pattern = array.to_array(vim.split(vim.fn.input("new pattern % "), " *:: *"))
...
function buffer_group.is_excluded_buffer(bufnr)
	local found = array.grep(dict.values(buffer_group.buffer_groups), function(obj)
		return obj.exclude[bufnr]
	end)

...
function buffer.setvar(bufnr, k, v)
    ...
    if is_a.string(k) then
        ...
        dict.each(k, function(key, value) buffer.setvar(bufnr, key, value) end)
    end
...
function buffer.setoption(bufnr, k, v)
    ...
    if is_a.string(k) then
        ...
        dict.each(
            k,
            function(key, value) buffer.setoption(bufnr, key, value) end
        )
    end
...
---  Is buffer visible?
--  @return boolean
function buffer.isvisible(bufnr) return vim.fn.bufwinid(bufnr) ~= -1 end
...
function buffer.info(bufnr, all)
    local function to_dict(lst)
        local new = {}
        array.each(lst, function(info) new[info.bufnr] = info end)
...
function buffer.list(criteria, opts)
    ...
    if not opts then return out end
    ...
    if name then out = array.map(out, buffer.name) end
    ...
    if filter then out = array.grep(out, filter) end

    if apply then out = array.map(out, apply) end

    if callback then callback(out) end

    if keep_dict then
        local info = {}
        array.each(out, function (bufnr) info[bufnr] = found[bufnr] end)
...
function buffer.botright(bufnr) return buffer.split(bufnr, "botright") end

function buffer.topleft(bufnr) return buffer.split(bufnr, "topleft") end

function buffer.rightbelow(bufnr) return buffer.split(bufnr, "belowright") end

function buffer.leftabove(bufnr) return buffer.split(bufnr, "aboveleft") end

function buffer.belowright(bufnr) return buffer.split(bufnr, "belowright") end

function buffer.aboveleft(bufnr) return buffer.split(bufnr, "aboveleft") end

function buffer.tabnew(bufnr) return buffer.split(bufnr, "t") end

function buffer.vsplit(bufnr) return buffer.split(bufnr, "v") end

...
telescope.load = function(override)
    ...
    return dict.merge({
        get_selected = function(self, bufnr, close)
            ...
            if close then self.actions.close(bufnr) end
            ...
            if mappings then
                ...
                opts.attach_mappings = function(prompt_bufnr, map)
                    if mappings then
                        ...
                        if default then
                            self.actions.select_default:replace(
                                function() default(prompt_bufnr) end
                            )
                        end
                        array.each(
                            array.rest(mappings),
                            function(x) map(unpack(x)) end
                        )
                    end
            ...
            return setmetatable({}, {
                __newindex = function(self, name, f)
                    rawset(self, name, function(bufnr)
                        array.each(
                            self:get_selected(bufnr, no_close),
        ...
        run_picker = function(...) return self:create_picker(...):find() end,
    }, opts)
end

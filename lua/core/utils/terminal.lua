...
function terminal.isrunning(self, success, failure)
	if pid_exists(self.pid) then
...
function terminal.unless_running(self, callback)
	return terminal.isrunning(self, nil, callback)
end

function terminal.if_running(self, callback)
	return terminal.isrunning(self, callback)
end
...
function terminal.send_current_line(self, src_bufnr)
	if not terminal.isrunning(self) then
		return
	end
...
function terminal.split(self, direction, opts)
	...
	if not terminal.isvisible(self) then
...
function terminal.float(self, opts)
	...
	if not terminal.isvisible(self) and self.buffer then
...
function terminal.stop(self)
	...
	if not self.pid then
		return false
	elseif not terminal.isrunning(self) then
		return false
	else
		kill_pid(self.pid, 9)
...
function terminal.isvisible(self)
	if self.buffer then
		return buffer.isvisible(self.buffer)
	end
	return false
end

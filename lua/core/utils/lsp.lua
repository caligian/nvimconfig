...
lsp.mappings = {
  ...
  lsp = {
    ...
    buffer_declarations = { "gD",    vim.lsp.buf.declaration,    { desc = "Buffer declarations" } },
    buffer_definitions = { "gd",    vim.lsp.buf.definition,     { desc = "Buffer definitions" } },
    float_documentation =  { "K",     vim.lsp.buf.hover,          { desc = "Show float UI" } },
    implementations =   { "gi",    vim.lsp.buf.implementation, { desc = "Show implementations" } },
    signatures =   { "<C-k>", vim.lsp.buf.signature_help, { desc = "Signatures" } }, 
    ...
    buffer_rename = { "<leader>lR", vim.lsp.buf.rename,      { desc = "Rename buffer" } },
    code_action = { "leaderla",   vim.lsp.buf.code_action, { desc = "Show code actions" } },
    buffer_references = {
      "gr",
...
function lsp.apply_mappings(overrides, callback)
  ...
  if overrides then dict.merge(mappings, overrides) end
...
function lsp.on_attach(client, bufnr)
  if client.name == "omnisharp" then
    ...
    if not has_formatter then lsp.attach_formatter(client) end
...
function lsp.setup_server(server, opts)
  ...
  if default_conf.cmd then default_conf.cmd = array.toarray(default_conf.cmd) end

...
function plugin.set_autocmds(self, autocmds)
	return au.map_group(string.upper(self.name), autocmds or self.autocmds or {})
end

function plugin.get(name, callback, args)
	local plugin = plugin.plugins[name] or plugin(name)
	callback = isstring(callback) and plugin[callback] or callback
...
function plugin.load_configs()
	if req2path("user.plugins") then
		...
		assert(istable(user_plugins), "plugins should be a dict containing specs")
...
function plugin.tolazy_spec()
	...
	if path.exists(user_path) then
		dict.merge(core, require("user.plugins"))
	end

	assert(not isempty(core), "lazy: expected non-empty dict")
...
function plugin.setup_lazy()
	require("lazy").setup(values(plugin.tolazy_spec()))
end

	...
	local self = {
		...
		set_autocmds = function(self, apply)
			...
			function autocmds.apply(event, opts)
				if current_apply then
					event, opts = current_apply(event, opts)
				end
				if apply then
			...
			self.autocmds = autocmd.map_group(name, autocmds)
		...
		config_exists = function(self, is_user)
			if is_user then
				return req2path(self.user_config_require_path)
			end
...
function plugin.get(name, callback)
	if not plugin.plugins[name] then
		return
	end
...
function plugin.to_lazy_spec()
	if dict.is_empty(plugin.plugins) then
		plugin.load_specs()
	end

...
local function resolve(ft_or_bufnr_self)
	if isstring(ft_or_bufnr_self) then
		return ft_or_bufnr_self
	elseif isobject(ft_or_bufnr_self, "filetype") then
		return ft_or_bufnr_self
	elseif isnumber(ft_or_bufnr_self) then
...
function filetype.create_actions_picker(bufnr)
	...
	if not specs or isempty(specs) then
		return false
	else
...
function filetype.set_autocmds(self, autocmds)
	...
	if not autocmds or isempty(autocmds) then
		return
	end
	...
	dict.each(autocmds, function(au_name, callback)
		...
		out[au_name] = au.map("FileType", { callback = callback, pattern = self.name, name = au_name })
	end)
...
function filetype.setup_lsp(self)
	...
	local server, config
	if isa.string(self.lsp_server) then
...
function filetype.au(self, opts)
	opts = opts or {}
	opts = deepcopy(opts)
	opts.group = self.augroup
	opts.pattern = self.name

	return au("FileType", opts)
end
...
function filetype.autocmds_with_opts(self, opts, autocmds)
	...
	au.map_with_opts(opts, autocmds)
end
...
function filetype.set_mappings(self, mappings)
	...
	if not mappings or isempty(self.mappings) then
		return
	end
	...
	dict.each(mappings, function(name, spec)
		...
		if opts then
			...
			rest = isa.string(rest) and { desc = rest } or rest
...
function filetype.format_dir(self, p, opts)
	...
	if exists and job.isactive(exists) then
	...
	if isa.list(args) and not formatter_config_path_exists(self.formatter_local_config_path) then
		cmd = concat(cmd .. " " .. args, " ")
	end
...
function filetype.format_buffer(self, bufnr, opts)
	...
	cmd = cmd .. " " .. bufname
	if stdin then
		cmd = sprintf('sh -c "cat %s | %s"', buffer.name(bufnr), cmd)
...
function filetype.load_spec(ft, isuser)
	local fname
...
function filetype.load_specs(isuser)
	local function name(f)
		return split(path.basename(f), "%.")[1]
	end

	if not isuser then
		list.each(dir.getallfiles(user.dir .. "/lua/core/ft/"), function(f)
			f = name(f)
			filetype.load_spec(f)
		end)
	else
		list.each(dir.getallfiles(user.user_dir .. "/lua/user/ft/"), function(f)
...
function filetype.command(self, attrib)
	return filetype.attrib(self, attrib, function(config)
		...
		return function(bufname)
			...
			bufname = bufname or buffer.name()
			asserttype(cmd, union("function", "table"))

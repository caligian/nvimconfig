...
function command.map_group(name, spec, compile)
  ...
  dict.map(spec, function(key, value)
    key = name .. capitalize(key)
    new[key] = value
    value[2] = dict.merge(copy(value[2] or {}), gopts)
